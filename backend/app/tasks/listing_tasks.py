...
async def submit_listing_task(product_id: str) -> dict:
    ...
    try:
        ...
        product_data = {
            ...
            # GTIN Exemption ALWAYS — no barcode fields sent
            ...
            "number_of_items": int(getattr(product, 'number_of_items', 1) or 1),
            "package_quantity": int(getattr(product, 'package_quantity', 1) or 1),
            # Images — parsed JSON list
            "images": _parse_json_field(product.images) if product.images else [],
        }

        # Ensure quantity is valid (Amazon rejects if quantity is missing/invalid)
